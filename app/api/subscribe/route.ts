...
export async function POST(request: NextRequest) {
  try {
    const { email, source = 'unknown', publication = 'general', first_name = null, last_name = null } = await request.json()
...
function generateWelcomeHTML({ first_name, source }: { first_name: string | null; source: string }) {
  const name = first_name ? ` ${first_name}` : ''
  return `
  <div style="font-family: Arial, sans-serif; line-height:1.6">
    <h2>Welcome${name} to AlphaB 🚀</h2>
    <p>Thanks for subscribing${source ? ` via <b>${source}</b>` : ''}. You'll receive updates on innovations, launches, and insights.</p>
...
function generateWelcomeText({ first_name, source }: { first_name: string | null; source: string }) {
  const name = first_name ? ` ${first_name}` : ''
  return `Welcome${name} to AlphaB!\n\nThanks for subscribing${source ? ` via ${source}` : ''}. You'll receive updates on innovations, launches, and insights.\n\nExplore RADE: https://rade.alphab.io\n\n— AlphaB`
}
