...
const FALLBACK_BASE_URL = (process.env.NEXT_PUBLIC_SITE_URL || "https://alphab.io").replace(
  /\/$/,
  ""
);
...
export function ShareButtons({ slug, title, description }: ShareButtonsProps) {
  ...
  const shareText = useMemo(() => {
    ...
    return truncatedDescription ? `${headline} - ${truncatedDescription}` : headline;
  }, [title, truncatedDescription]);

  const encodedUrl = useMemo(() => encodeURIComponent(shareUrl), [shareUrl]);
  const encodedTitle = useMemo(() => encodeURIComponent(title?.trim() || ""), [title]);
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {copied && <span className="text-xs text-blue-600 dark:text-blue-400 ml-1">Link copied</span>}
    </div>
  );
}
