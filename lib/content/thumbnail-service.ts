...
export class ThumbnailService {
  ...
  static generateThumbnail(options: ThumbnailOptions): string {
    ...
    const extractedImage = this.tryExtractImage(options.url)
    if (extractedImage) return extractedImage

    // 2. Generate dynamic gradient thumbnail
    return this.generateGradientThumbnail(options)
  }
  ...
  private static generateGradientThumbnail(options: ThumbnailOptions): string {
    ...
    const sourceColors = this.getSourceColors(options.source)
    ...
    const gradient = this.createGradient(bgColor, accentColor, options.title)
    
    // Return a data URL for the generated image
    return this.generateDataURL(gradient, options)
  }
  ...
  private static getSourceColors(source: string) {
    return this.SOURCE_COLORS[source as keyof typeof this.SOURCE_COLORS]
  }

  /**
   * Create gradient pattern based on title
   */
  private static createGradient(bgColor: string, accentColor: string, title: string): string {
   ...
   */
  private static generateDataURL(gradient: string, options: ThumbnailOptions): string {
    // For production, this would generate an actual image
    // For now, return a CSS gradient as background
    return `data:image/svg+xml,${encodeURIComponent(this.generateSVG(gradient, options))}`
  }

  /**
   * Generate SVG thumbnail
   */
  private static generateSVG(gradient: string, options: ThumbnailOptions): string {
    const colors = this.getCategoryColors(options.category)
    const sourceColors = this.getSourceColors(options.source)
    const bgColor = sourceColors?.bg || colors.bg
    const accentColor = sourceColors?.accent || colors.accent
    ...
    const icon = this.getCategoryIcon(options.category)
    
    // Truncate title for display
    const displayTitle = options.title.length > 40 
   ...
   */
  private static getCategoryIcon(category: ThumbnailOptions['category']): string {
    const icons = {
   ...
   */
  static getUnsplashImage(tags: string[] = [], width = 400, height = 200): string {
    ...
    return `https://source.unsplash.com/collection/${this.UNSPLASH_COLLECTION_ID}/${width}x${height}/?${searchQuery}`
  }
  ...
  static getBestThumbnail(options: ThumbnailOptions): string {
    ...
    if (this.UNSPLASH_ACCESS_KEY && options.tags) {
      const unsplashImage = this.getUnsplashImage(options.tags)
