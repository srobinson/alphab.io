...
import { RSSParser, type RSSItem, type FetchResult } from './rss-parser'
import { ContentClassifier, type ClassifiedContent } from './classifier'
import { getActiveSourcesByPriority, type ContentSource } from './sources'
...
export class ContentSyncService {
  ...
  async syncAllSources(options: SyncOptions = {}): Promise<SyncResult[]> {
    ...
    console.log(`Starting sync for ${sources.length} sources`)
    ...
    console.log(`Sync completed: ${successCount}/${results.length} sources successful, ${totalFetched} items fetched, ${totalIngested} items ingested`)
    
    return results
  }

  private async syncSource(source: ContentSource, options: SyncOptions): Promise<SyncResult> {
    ...
    try {
      ...
      console.log(`Processing ${itemsToProcess.length}/${fetchResult.items.length} items from ${source.name}`)
      ...
      for (const item of itemsToProcess) {
        try {
          ...
          console.warn(`Failed to ingest item ${item.title}:`, error)
        }
      }
  ...
  private async updateIndustryMovesCache(): Promise<void> {
    try {
      ...
      const trendingContent = await this.classifier.getTrendingContent(rssItems, 8)
      ...
      if (cacheEntries.length > 0) {
        ...
        console.log(`Updated industry moves cache with ${cacheEntries.length} items`)
      }
