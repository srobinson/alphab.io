...
export function normalizeUrl(input: string): string {
  try {
    ...
    const paramsToDrop = [
      'utm_source','utm_medium','utm_campaign','utm_term','utm_content','utm_id','gclid','fbclid','mc_cid','mc_eid'
    ]
...
export async function fetchMetadata(url: string): Promise<PageMeta> {
  ...
  const ogTitle = extractBetween(html, /<meta[^>]*property=["']og:title["'][^>]*content=["']([^"']+)["'][^>]*>/i)
  const ogDesc = extractBetween(html, /<meta[^>]*property=["']og:description["'][^>]*content=["']([^"']+)["'][^>]*>/i)
...
export async function ingestUrl(input: IngestInput, sb: SupabaseClient): Promise<IngestResult> {
  ...
  const finalTags = (Array.isArray(input.tags) && input.tags.length)
    ? input.tags
    : (inferredTags.length ? inferredTags : (existing?.tags ?? []))
  const finalSource = input.source || inferredSourceLabel || existing?.source || source
  const finalSummary = input.doSummarize ? (summary ?? existing?.summary ?? null) : (existing?.summary ?? null)
  const finalPublishedAt = meta.publishedAt
    ? new Date(meta.publishedAt).toISOString()
    : (existing?.published_at ?? null)
  const finalContentHtml = (input.doSaveContent && meta.contentHtml)
    ? meta.contentHtml
