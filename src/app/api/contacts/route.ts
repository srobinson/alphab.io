 ...
 * POST /api/contacts
 *
 * Handles contact form submissions with rate limiting and email notifications.
 *
 * @param request - Contact form data (name, email, message)
 * @returns Contact ID on success, error message on failure
 */
export async function POST(request: NextRequest): Promise<NextResponse<ContactResponse>> {
  ...
  try {
    ...
    const sanitizedEmail = email.trim().toLowerCase().slice(0, 254); // Limit email length
    const sanitizedMessage = message.trim().slice(0, 5000); // Limit message length
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: "Failed to store contact submission. Please try again later." },
        { status: 500 }
      );
    }
