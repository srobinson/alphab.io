...
export async function fetchNewsAPI(query: string, pageSize = 10): Promise<NewsItem[]> {
  ...
  try {
    const url = `${NEWS_API_CONFIG.baseUrl}/everything?q=${encodeURIComponent(
      query
    )}&sortBy=publishedAt&pageSize=${pageSize}&apiKey=${NEWS_API_CONFIG.apiKey}`;
...
export async function fetchGNewsAPI(query: string, max = 10): Promise<NewsItem[]> {
