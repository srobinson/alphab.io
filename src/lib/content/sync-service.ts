...
import { getUnsplashSearchQuery } from "./simple-thumbnails";
...
export class ContentSyncService {
      ...
      for (const item of itemsToProcess) {
        try {
          ...
          if (!options.disableImages) {
            ...
            if (!imageUrl) {
              const searchQuery = getUnsplashSearchQuery(item.title, item.classification.tags);
  ...
  async testConnection(): Promise<boolean> {
    try {
      const { error } = await this.supabase.from("articles").select("id").limit(1);
