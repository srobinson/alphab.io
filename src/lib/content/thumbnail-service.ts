...
const UNSPLASH_COLLECTION_ID = "1516642"; // AI/Technology collection
const UNSPLASH_ACCESS_KEY = "your-unsplash-access-key"; // Optional
...
export function generateThumbnail(options: ThumbnailOptions): string {
  ...
  const extractedImage = tryExtractImage(options.url);
  if (extractedImage) return extractedImage;

  // 2. Generate dynamic gradient thumbnail
  return generateGradientThumbnail(options);
}

/**
 * Extract image from RSS content or URL
 */
function tryExtractImage(url?: string): string | null {
  if (!url) return null;
 ...
 */
function generateGradientThumbnail(options: ThumbnailOptions): string {
  const colors = getCategoryColors(options.category);
  const sourceColors = getSourceColors(options.source);
  ...
  const gradient = createGradient(bgColor, accentColor, options.title);

  // Return a data URL for the generated image
  return generateDataURL(gradient, options);
}

/**
 * Get category-based colors
 */
function getCategoryColors(category: ThumbnailOptions["category"]) {
 ...
 */
function createGradient(bgColor: string, accentColor: string, title: string): string {
 ...
 */
function generateDataURL(gradient: string, options: ThumbnailOptions): string {
  // For production, this would generate an actual image
  // For now, return a CSS gradient as background
  return `data:image/svg+xml,${encodeURIComponent(generateSVG(gradient, options))}`;
}

/**
 * Generate SVG thumbnail
 */
function generateSVG(_gradient: string, options: ThumbnailOptions): string {
  const colors = getCategoryColors(options.category);
  const sourceColors = getSourceColors(options.source);
  const bgColor = sourceColors?.bg || colors.bg;
  const accentColor = sourceColors?.accent || colors.accent;
  ...
  const icon = getCategoryIcon(options.category);

  // Truncate title for display
  const displayTitle =
    options.title.length > 40 ? `${options.title.substring(0, 40)}...` : options.title;
 ...
 */
function getCategoryIcon(category: ThumbnailOptions["category"]): string {
  const icons = {
 ...
 */
export function getUnsplashImage(tags: string[] = [], width = 400, height = 200): string {
  if (!UNSPLASH_ACCESS_KEY) {
    return ""; // Fallback to generated thumbnail
  }

  // Common AI/Tech keywords for Unsplash search
  const searchTerms = ["artificial-intelligence", "technology", "computer", "data", "digital"];
  const matchingTerms = tags.filter((tag) => searchTerms.includes(tag.toLowerCase()));
  const searchQuery = matchingTerms.length > 0 ? matchingTerms[0] : "technology";

  return `https://source.unsplash.com/collection/${UNSPLASH_COLLECTION_ID}/${width}x${height}/?${searchQuery}`;
}

/**
 * Get placeholder image with better variety
 */
export function getPlaceholderImage(options: ThumbnailOptions): string {
...
export function getBestThumbnail(options: ThumbnailOptions): string {
  ...
  if (UNSPLASH_ACCESS_KEY && options.tags) {
    const unsplashImage = getUnsplashImage(options.tags);
