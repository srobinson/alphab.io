        ...
        messages: [
          {
            role: "system",
            content:
              "You are a concise editor creating business-focused summaries for AI-curious executives. Limit to 1-2 sentences. No fluff.",
          },
          {
            role: "user",
...
export async function summarizeInfo(input: SummarizeInfoInput): Promise<SummarizeInfoOutput> {
  ...
  try {
    ...
    const tags = Array.isArray(parsed.tags)
      ...
      : [];
    const sourceLabel =
      typeof parsed.sourceLabel === "string" ? parsed.sourceLabel.trim() : undefined;
    return sourceLabel ? { summary, tags, sourceLabel } : { summary, tags };
  } catch {
