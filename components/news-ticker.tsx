...
import { AlertCircle, TrendingUp, Zap, Radio, Clock } from "lucide-react"
...
const TickerItem = ({ item }: { item: NewsItem }) => {
  ...
  return (
    ...
    >
      ...
      <span className={`text-gray-700 dark:text-gray-200 transition-all duration-150`}>{item.text}</span>
      <span className="ml-2 text-xs text-gray-500 dark:text-gray-400 font-mono whitespace-nowrap">{item.time}</span>
      {item.isRSS && <span className="ml-1 text-xs text-green-600 dark:text-green-400 font-bold">●</span>}
    </span>
  )
}
  ...
  if (hasError) {
    return (
      <div className="flex items-center justify-center py-2">
        <span className="text-sm text-gray-600 dark:text-gray-400">News ticker temporarily unavailable</span>
      </div>
    )
  }
    ...
    <div className="w-full">
      <div className="flex flex-col w-full">
        ...
        <div className="flex-grow bg-gray-200/95 dark:bg-gray-800/95 backdrop-blur-sm overflow-hidden py-2 w-full">
          {isLoading ? (
            <div className="flex items-center justify-center py-2">
              <span className="text-sm text-gray-600 dark:text-gray-400">Loading latest news...</span>
            </div>
          ) : (
            <MarqueeWrapper>
              {newsItems.map((item) => (
