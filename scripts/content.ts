...
async function runSync(
	...
	try {
		...
		for (const result of results) {
			...
			console.log(
				`   Items: ${result.itemsFetched} fetched, ${result.itemsIngested} ingested`,
			);
		...
		console.log(
			`   Content: ${totalFetched} fetched, ${totalIngested} ingested`,
		);
		console.log(
...
async function showStatus() {
	...
	try {
		...
		for (const source of CONTENT_SOURCES) {
			...
			console.log(
				`  ${status} ${source.name.padEnd(25)} ${priority} ${source.category}`,
			);
		}
