...
import { ingestUrl, type IngestResult } from '../lib/server/ingest'
...
async function main() {
  ...
  const url = args._[0]
  if (!url) {
    console.error('Usage: tsx scripts/ingest.ts <url> [--source <name>] [--tags tag1,tag2] [--no-summarize] [--save-content]')
    process.exit(1)
  }
  ...
  const tags = typeof tagsFlag === 'string' && tagsFlag.length
    ? tagsFlag.split(',').map((s) => s.trim()).filter(Boolean)
    : []
  const doSummarize = args.flags['no-summarize'] === 'true' ? false : true
  const doSaveContent = args.flags['save-content'] === 'true'
  const source = args.flags['source']

  console.log('→ Ingesting', url, 'source=', source || '(hostname)', 'summarize=', doSummarize, 'saveContent=', doSaveContent, 'tags=', tags)
